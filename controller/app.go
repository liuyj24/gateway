...
func (admin *APPController) AppAdd(c *gin.Context) {
	...
	middleware.ResponseSuccess(c, "add successfully!")
	return
}
...
func (admin *APPController) AppStatistics(c *gin.Context) {
	...
	for i := 0; i <= currentTime.In(lib.TimeLocation).Hour(); i++ {
		...
		todayStat = append(todayStat, int64(i*1))
	}
	...
	for i := 0; i <= 23; i++ {
		...
		//hourData, _ := counter.GetHourData(dateTime)
		yesterdayStat = append(yesterdayStat, int64(i*2))
	}
